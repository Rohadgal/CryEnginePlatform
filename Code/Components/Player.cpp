...
CPlayerComponent::CPlayerComponent() :
	...
	m_airSpeed(10.0f),
	m_jumpHeight(2.5f),
	m_cameraOffsetForward(-4.5f),
	m_cameraOffsetUp(2.8f)
{
	REGISTER_CVAR2("g_moveSpeed", &m_moveSpeed, m_moveSpeed, VF_NULL, "Speed the player moves");
	REGISTER_CVAR2("g_sprintSpeed", &m_sprintSpeed, m_sprintSpeed, VF_NULL, "Speed the player sprints");
	REGISTER_CVAR2("g_airSpeed", &m_airSpeed, m_airSpeed, VF_NULL, "Speed the player moves in air");
	REGISTER_CVAR2("g_jumpHeight", &m_jumpHeight, m_jumpHeight, VF_NULL, "Jump height of the player");
	REGISTER_CVAR2("g_cameraOffsetForward", &m_cameraOffsetForward, m_cameraOffsetForward, VF_NULL, "Offset from player in forward direction");
	REGISTER_CVAR2("g_cameraOffsetUp", &m_cameraOffsetUp, m_cameraOffsetUp, VF_NULL, "Offset from player in the up direction");
}

CPlayerComponent::~CPlayerComponent() {
	gEnv->pConsole->UnregisterVariable("g_moveSpeed");
	gEnv->pConsole->UnregisterVariable("g_sprintSpeed");
	gEnv->pConsole->UnregisterVariable("g_airSpeed");
	gEnv->pConsole->UnregisterVariable("g_jumpHeight");
	gEnv->pConsole->UnregisterVariable("g_cameraOffsetForward");
	gEnv->pConsole->UnregisterVariable("g_cameraOffsetUp");
}
...
void CPlayerComponent::InitializeLocalPlayer() {
	...
	m_pInputComponent->RegisterAction("player", "sprint", [this](int activationMode, float value) {HandleInputFlagChange(EInputFlag::Sprint, (EActionActivationMode)activationMode); });
...
void CPlayerComponent::UpdateMovementRequest(float frameTime)
{
	...
	else if (m_inputFlags & EInputFlag::Sprint)
	{
	...
	if (m_pCharacterController->IsOnGround() && m_inputFlags & EInputFlag::Jump) {
		Vec3 jumpVelocity = Vec3(0, 0, m_jumpHeight);
...
void CPlayerComponent::UpdateCamera(float frameTime)
{
	...
	Matrix34 rotationMatrix = CCamera::CreateOrientationYPR(ypr);



	// Offset the player along the forward axis (normally back)
	// Also offset upwards
	Matrix34 translationMatrix = Matrix34::CreateTranslationMat(Vec3(0, m_cameraOffsetForward, m_cameraOffsetUp));
	Matrix34 finalTransform = rotationMatrix * translationMatrix;

	m_pCameraComponent->SetTransformMatrix(finalTransform);
	m_pAudioListenerComponent->SetOffset(finalTransform.GetTranslation());
}
